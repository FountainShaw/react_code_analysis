...
那这又引出另一个问题，我们怎么让原先的迭代代码突然停止，而又再某个特定的时候再次开始执行？
其实迭代我们倒是可以把他进行拆解，分成很多细微的任务，每个任务就执行很短时间
但是什么时候去执行优先级高的任务呢？事实上我们可以一直执行优先级高的任务
当这些任务执行的差不多了，再通知那些被拆分的细微任务接着执行
...
其实说白了，这个API，就类似一个setTimeout，只是不用你控制主线程啥时候该执行回调，
而是当主线程一空闲下来，他就会主动的去执行回调了。而且他为回调函数提供了一个参数deadline。
